...
export default function OptionCard({ optionData }) {
    ...
    return (
        <div className="shadow-md bg-white flex flex-col items-center justify-center gap-4 p-6 rounded">
            ...
            {enabled ? (
                <Link href={link} className='py-2 rounded-sm bg-blue-600 px-3 inline-flex items-center space-x-2 text-white'>
                    {linkTitle}
                </Link>
            ) : (
                <button href={link} className='py-2 rounded-sm bg-blue-600 px-3 inline-flex items-center space-x-2 text-white'>
                    Enable
                </button>            
            )}
